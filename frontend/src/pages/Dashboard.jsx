        ...
        <div className="mb-6">
          ...
          <div className="hidden lg:flex lg:items-center lg:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Guest bet button is always available until week is settled */}
              {!isSettled && (
                <button
          ...
          <div className="lg:hidden space-y-3">
            ...
            {/* Row 2: Buttons - Place Bet only when not locked, Add Guest always until settled */}
            <div className={`${!lockStatus.isBettingLocked && !isSettled ? 'grid grid-cols-2 gap-3' : 'flex gap-3'}`}>
              {!lockStatus.isBettingLocked && (
                <Link
                  to="/place-bet"
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-emerald-600 hover:bg-emerald-700 text-white transition-colors flex-1"
                >
                  <BetIcon />
                  <span>{bets.some(bet => {
                    const odId = user?._id || user?.id
              ...
              {!isSettled && (
                <button
                  onClick={handleOpenGuestModal}
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-purple-600 hover:bg-purple-700 text-white transition-colors flex-1"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                  </svg>
